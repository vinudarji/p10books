...
class ImageComponent extends Object { 
                ...
                if(array_key_exists('error', $file) AND $file['error'] === 0) { 
                    ...
                    elseif(!in_array($file['type'], $this->allowed_mime_types))  
                        ...
                        if(in_array($extension, $this->allowed_extensions)) { 
                            // generate extension 
														$save_filename = md5(microtime()) . '.' . $extension;
                            ...
                            if(file_exists($destination)) { 
                                ...
                                $this->file_path = $destination; 
																return array('filepath' => $destination,'filename' => $save_filename); 
    ...
    public function thumb($file) { 
        ...
        $pathinfo = pathinfo($file); 
				$save_filename = md5($pathinfo['filename'] . $this->width . $this->height . $this->zoom_crop) . 
        '.' . $pathinfo['extension'];

        $destination = $this->save_paths['thumb'] . $save_filename; 
